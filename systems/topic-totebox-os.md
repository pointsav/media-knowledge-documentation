...
The architecture rejects traditional role-based authorization in favor of **Machine Authorization** and **Device-based Permissions**. Every transaction on a PointSav appliance is cryptographically anchored to a WORM ledger, ensuring that only authorized hardware devices can modify the system state.

## Deployment model

A Totebox deployment operates as a **Freely Transferable** system. It is delivered as a **Bootable Disk Image** that can be deployed on Bare metal resources or public cloud resources without structural dependency on the hosting provider.
