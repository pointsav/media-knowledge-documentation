...
foundation on which the Capability Ledger Substrate is built:
...
The substrate does not modify the seL4 kernel. It builds on the kernel's
...
x86-64 is not excluded — seL4 supports it — but AArch64-first means that the
...
demonstrated on the workspace VM.
...
- [[moonshot-toolkit-build-orchestrator]] — the Rust build orchestrator that
  cross-compiles protection domains and assembles this target's boot image
- [[capability-ledger-substrate]] — what the unikernel images enforce at runtime
