---
schema: foundry-doc-v1
title: "PointSav Media Kit"
slug: pointsav-media-kit
category: governance
status: active
last_edited: 2026-05-25
editor: pointsav-engineering
---

## About PointSav Digital Systems

PointSav Digital Systems builds operating systems and services for regulated businesses
that need to own their data, their AI, and their record-keeping outright. The platform
runs on customer hardware and produces continuous-disclosure-grade audit trails.

PointSav is a wholly-owned subsidiary of Woodfine Capital Projects Inc.

## Brand and Wordmarks

**PointSav Digital Systems™** and the PointSav wordmark are trademarks of
Woodfine Capital Projects Inc., used in Canada, the United States, Latin America,
and Europe.

Use of the PointSav wordmark in press and media requires written permission from
Woodfine Capital Projects Inc. For inquiries, contact
[open.source@pointsav.com](mailto:open.source@pointsav.com).

## Press Inquiries

Media and press inquiries should be directed to Woodfine Capital Projects Inc.

- **General contact:** [open.source@pointsav.com](mailto:open.source@pointsav.com)
- **Newsroom:** [woodfinegroup.com](https://woodfinegroup.com)

## Licensing

The content of this wiki is licensed under
[Creative Commons Attribution 4.0 International (CC BY 4.0)](https://creativecommons.org/licenses/by/4.0/).
You are free to share and adapt the material for any purpose, provided you give
appropriate credit to PointSav Digital Systems.

Source code for the PointSav platform is available at
[github.com/pointsav](https://github.com/pointsav) under the terms stated in each
repository's LICENSE file.

## Trademarks

Woodfine Capital Projects™, MCorp™, PointSav Digital Systems™,
Totebox Orchestration™, Totebox Archive™, and Capability Geometry™ are trademarks of Woodfine Capital
Projects Inc. All other trademarks are the property of their respective owners.
