...
`bin/mailbox-prepend.sh` resuelve esto con `flock`. Toma la ruta del buzón de destino, deriva una ruta de bloqueo en `.agent/locks/` para buzones de alcance archivado (o `/tmp/foundry-mailbox-<sha>.lock` para casos ad-hoc) y adquiere un bloqueo exclusivo con un tiempo de espera de 30 segundos antes de realizar la lectura-modificación-escritura. Dos llamadas simultáneas se serializan en lugar de colisionar.
