...
El punto de entrada previsto es el comando de consola `open-archive`, planificado para invocarse desde la sesión concentradora. Cuando se invoque, está previsto que:

1. Lea el manifiesto del archivo
2. Muestre el nombre del archivo, el estado de la tétrada, el punto final de la pasarela de IA y el recuento de mensajes pendientes en el buzón de entrada
...
- Escribe archivos de configuración o herramientas a nivel del espacio de trabajo
- Escribe en el almacén de identidades
- Accede directamente a los árboles fuente canónicos (esos reciben contenido por promoción, no por escritura directa)
...
La sesión concentradora ratifica la completitud de la tétrada cuando las cuatro patas están presentes.
...
- Las solicitudes salientes a otros archivos o a la sesión concentradora se anteponen en `.agent/outbox.md` (más reciente arriba)
- La sesión concentradora barre los buzones de salida del clúster durante la revisión del espacio de trabajo y enruta los mensajes
...
|---|---|---|
| P1 | Administrador del sistema | Concentrador + cada archivo + cada nodo de infraestructura |
| P2 | Gerente de paquetes | Archivos específicos más rutas de promoción de la etapa 6 |
| P3 | Usuario | Archivos específicos; sin emparejamiento con el concentrador; solo lectura entre archivos |
| P4 | Interfaz | Solo superficie de API de solo lectura |

Los niveles se aplican mediante emparejamientos, no comparaciones de cadenas. El `os-console` de un colaborador P3 no está emparejado con el concentrador — la conexión no existe. Véase [[pairing-as-permission]] para el mecanismo formal.
