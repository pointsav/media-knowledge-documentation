...
<!--claim id=hot-pluggable-adapter confidence=structural cites=[]-->El Diodo lo aplica un pequeño servicio conectable en caliente, `service-pointsav-link` (el paquete `pointsav-protocol`). Es el único código que traduce comandos de autoridad en operaciones ejecutables por el Sujeto.<!--/claim-->
...
<!--claim id=universal-standard confidence=structural cites=[]-->El Diodo no es una característica del MediaKit ni del Totebox; se aplica de forma idéntica a cada sistema operativo `os-*`. El mismo paquete `service-pointsav-link`, con diferentes enlaces de política, se sitúa entre cualquier par de nodos que se comuniquen.<!--/claim-->
