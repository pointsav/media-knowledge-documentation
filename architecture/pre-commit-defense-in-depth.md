...
The Phase 1 hardening closes this with a single pre-commit hook: `bin/pre-commit-foundry-gate.sh`, symlinked into `.git/hooks/pre-commit` at the workspace root and every archive clone. It runs three checks against every commit.

**Helper-only commits.** The hook refuses unless `FOUNDRY_COMMIT_HELPER=1` is set in the environment. `bin/commit-as-next.sh` sets this variable before invoking `git commit`. Direct `git commit` invocations — by an operator, an agent, or automation — receive a clear error message pointing at the helper. Merge commits and rebases are allowed via `GIT_REFLOG_ACTION` detection; emergencies can bypass with `--no-verify`. The cost of the rule is one redirected commit; the benefit is the end of the entire "wrong author" class.

**Secret-pattern scan.** The hook reads `conventions/secret-patterns.yaml` and scans staged content against 17 regex patterns: SSH/PGP private keys (OPENSSH, RSA, EC, DSA, PGP), cloud credentials (AWS, GCP, GitHub PAT, OAuth tokens), API keys (Anthropic, OpenAI, Slack), and generic password assignments. Critical and high-severity matches block the commit; medium and low produce a warning and proceed. A path allowlist exempts the pattern catalogue itself and public-key files.
...
What this does not solve: bypass via `--no-verify`, or a commit on a workspace clone without the hook installed. The first is intentional — an operator with a legitimate reason can bypass with a full audit log entry. The second is closed by `bin/onboarding/new-archive.sh`, which installs the hook symlink whenever an archive is provisioned.
