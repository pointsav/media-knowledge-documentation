...
The PPN and the systems built on top of it are organised into four layers. Each layer is blind to the internal state of the layers below and above it.
...
This means a compromise of the routing layer — the PPN — yields no access to archive contents. The two planes are structurally blind to each other.
...
The disk image is the archive. The resource pool is node infrastructure. Moving the image moves the archive.
...
This separation is intentional: the network control plane and the data access plane are designed so that a failure or compromise of one does not cascade to the other.
