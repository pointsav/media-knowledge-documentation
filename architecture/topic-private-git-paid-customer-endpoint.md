...
short_description: "The binary release server that delivers compiled PointSav software to customers who hold valid Ed25519 license tokens, performing all verification locally without external service calls."
...
paired_with: topic-private-git-paid-customer-endpoint.es.md
cites: []
---
...
The service is `app-privategit-source`, deployed on `vault-privategit-source-1`.
...
4. Checks that `product_id` and `version_constraint` in the token cover the requested
   binary.

Step 4 is the authorization decision: a token purchased for product A does not grant
access to product B, and a token for version 1.x does not grant access to version 2.0.
...
The binary server performs all verification without contacting the marketplace or any
external service at request time. The license registry (a local key store populated from
the marketplace at publication time) is the only dependency for token verification. This
means the binary server can serve downloads during marketplace downtime, and it can be
deployed in a network-isolated environment.

## License Registry Sync

The local license registry is populated when the marketplace publishes a new binary or
when a license token is issued. Registry synchronization between the marketplace and the
binary server is intended for a future phase; in the current phase, the registry is
populated by direct write at the time of marketplace-side issuance.

## Token Lifecycle

Tokens may be perpetual (`expires_at = 0`) or time-limited. The binary server enforces
expiry at the time of each request. Token revocation via a deny-list of token IDs is
intended for a future phase; in the current phase the binary server does not maintain a
revocation list, so a token remains usable until its expiry timestamp.

Customers with expired tokens must renew through `app-privategit-marketplace`. Renewal
issues a new token.

## See Also

- [[topic-software-distribution-substrate|Software Distribution Substrate]] — full
  component inventory including the marketplace and payment watcher
- [[topic-crypto-license-sales-architecture|Crypto License Sales Architecture]] — how
  a license token is generated and delivered before it reaches this endpoint
