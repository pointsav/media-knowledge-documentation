...
The Doorman Protocol describes the routing and audit discipline implemented by `service-slm`, the sole service in Ring 3 of the [[three-ring-architecture]]. Every inference call that leaves a Totebox passes through one service at one boundary — the Doorman — which enforces sanitise-and-rehydrate discipline, routes to the appropriate compute tier, logs every event to an immutable audit ledger, and accumulates the training signal that compounds the substrate over time.

## Why a Doorman

The boundary problem: a Totebox holds the customer's authoritative structured data; external compute (large language models) cannot be trusted with raw structured facts. Without a single boundary, every service in the Totebox grows its own egress path, every egress path needs its own audit, and the sanitise-and-rehydrate discipline (SYS-ADR-07) becomes per-service discipline rather than substrate discipline. The Doorman centralises the boundary so the discipline is enforced once.
